...
RSpec.describe SessionsController, type: :controller do
  let(:my_user) { User.create!(name: "Blochead", email: "blochead@bloc.io", password: "password") }
     ...
     describe "WIKI sessions" do
       it "returns http success" do
         wiki :create, params: {session: {email: my_user.email}}
         expect(response).to have_http_status(:success)
       end

       it "initializes a session" do
         wiki :create, params: {session: {email: my_user.email, password: my_user.password}}
       ...
       it "flashes #error with bad email address" do
         wiki :create, params: {session: {email: "does not exist"}}
